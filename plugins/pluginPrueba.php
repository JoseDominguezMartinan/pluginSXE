...
// obtenemos los resultados de la consulta 
	...
	// almacenamos los datos para la sustitucion 
	$search=Array();
	$replace=Array();
	
	// en caso de que se hayan devuelto resultados 
  if(count($consulta)!=null){
  	//recorremos los resultados
		for($i=0;$i<count($consulta);$i++){
			// si el post_title coincide con uno anterior marcamos coincide a true
			$search[$i]=$consulta[$i]->post_title;
			if($search[$i]==$title)
				$coincide=true;
			else
			$coincide=false;
			//en caso de coincidencia cambiamos el titulo por titulo mas un numero al final 
			if($coincide==false){
				$replace[$i]=$search[$i];
				$titulo=$replace[$i];
			}
...
// se usaron shortcodes para esta comprobacion y asi poder hacer el paso anterior 
